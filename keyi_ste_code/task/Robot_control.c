...
void Robot_control(void)
{
	  Shoot_control();//射击控制
		...
		if(DevicesGet_State(DR16_MONITOR) != Off_line && Chassis_start == true)//遥控器离线时不发送数据给底盘
		{
...
void SendMsgtoCHAS(void)
{
		  ...
		  int16_t  tempSendData[8] = {0};
							...
							//--- 上层主控计算出的底盘速度
			tempSendData[0] = (int16_t)DR16_Get_ExptVx() >> 8;
			tempSendData[1] = (int16_t)DR16_Get_ExptVx();

			tempSendData[2] = (int16_t)DR16_Get_ExptVy() >> 8;
			tempSendData[3] = (int16_t)DR16_Get_ExptVy();

			tempSendData[4] = (int16_t)DR16_Get_ExptVw() >> 8;
			tempSendData[5] = (int16_t)DR16_Get_ExptVw();	
