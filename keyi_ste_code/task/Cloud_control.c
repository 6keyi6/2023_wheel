...
void YAW_PID_Init(void)
{
	  ...
	  Yaw_Vision_IN_Pid.I_SeparThresh = 200;
		...
		PositionPID_paraReset(&Yaw_Vision_Out_Pid_t,0.5f,0,0,29000,20000);  //自瞄
	  PositionPID_paraReset(&Yaw_Vision_IN_Pid_t,250.f,6.f,0,29000,20000);
	  Yaw_Vision_IN_Pid_t.I_SeparThresh = 200;
	
		PositionPID_paraReset(&Yaw_Vision_Out_Pid_spin,0.5f,0,0,29000,20000);  
	  PositionPID_paraReset(&Yaw_Vision_IN_Pid_spin,250.f,6.f,0,29000,20000);//小陀螺
...
{	
       //1 140
			 ...
			 PositionPID_paraReset(&M6020_PIT_IN,200,0.0f,0.f,29990,10000); //150
		 ...
		 /***********************自瞄PID初始化*****************************/
			 ...
			 PositionPID_paraReset(&PIT_Vision_Out_Pid_t,0.6f,0.0f,0.0f,29000,10000);//自瞄
			 PIT_Vision_Out_Pid_t.I_SeparThresh = 0;
			 PositionPID_paraReset(&PIT_Vision_IN_Pid_t,180,1.f,0.f,29990,20000); 
			 ...
			 PositionPID_paraReset(&PIT_Vision_IN_Pid_spin,180,1.f,0.f,29990,20000); 
...
//    Pit_AngleLimit_1(&PITCH_Use.RC_PICTH);//pit角度限制新换
...
float Last_Max = 0;
float Last_Lim = 0;
float Last_Max_Con = 5700;
float Cloud_Pitch_Max= 5700;
float Cloud_Pitch_Min = 4800;
...
//  }
	 ...
	 Cloud_Pitch_Max = 5700;
	 
   IMUMode_Limit_L = DJIC_IMU.pitch * 22.7527f + fabs(Cloud_getPitchAngleWithDown());
   IMUMode_Limit_H = DJIC_IMU.pitch * 22.7527f - fabs(Cloud_getPitchAngleWithUp());
    
 
    if (M6020moto_chassis[3].total_angle < Cloud_Pitch_Min && Last_Lim != *PitchAngle)
    {
   ...
   else
   {
    ...
    if (M6020moto_chassis[3].total_angle > Cloud_Pitch_Max && (Last_Max != *PitchAngle || Last_Max_Con != Cloud_Pitch_Max))
    {
...
{
    if (M6020moto_chassis[3].total_angle < Cloud_Pitch_Max)
    {
        return 0;
    }
    else
    {
        return (M6020moto_chassis[3].total_angle - Cloud_Pitch_Max);
    }
}
 float Cloud_getPitchAngleWithDown(void)
{
    if (M6020moto_chassis[3].total_angle > Cloud_Pitch_Min)
    {
        return 0;
    }
    else
    {
        return (M6020moto_chassis[3].total_angle - Cloud_Pitch_Min);
    }
}
...
{
    return ((M6020moto_chassis[3].total_angle>GIMBAL_PIT_MAX)? 0 : M6020moto_chassis[3].total_angle - GIMBAL_PIT_MAX);
}
float Get_PitDeviateL(void)
{
    return ((M6020moto_chassis[3].total_angle<GIMBAL_PIT_MIN)? 0 : M6020moto_chassis[3].total_angle - GIMBAL_PIT_MIN);
}


float Get_PitDeviateH1(void)
{
    return ((M6020moto_chassis[3].total_angle>GIMBAL_PIT_MAX1)? 0 : M6020moto_chassis[3].total_angle - GIMBAL_PIT_MAX1);
}
float Get_PitDeviateL1(void)
{
    return ((M6020moto_chassis[3].total_angle<GIMBAL_PIT_MIN1)? 0 : M6020moto_chassis[3].total_angle - GIMBAL_PIT_MIN1);
}
/**
 * @brief  	   激光红点控制
 * @param[in]  state
