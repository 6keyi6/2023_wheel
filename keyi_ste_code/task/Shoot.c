...
int get_time;//热量限制时间
// --- 摩擦轮转速
int16_t FricSpeed_15 = 4120;//4220
int16_t FricSpeed_18 = 4630;//4500
int16_t FricSpeed_22 = 4700;
int16_t FricSpeed_30 = 7050;//7550
...
void AttackMode_Ctrl(void)
{
    ...
    case Attack_30:
        ...
        if(VisionData.RawData.whether_Fire != false)
        {	
        ...
        if ((Ashoot_Cnt%=SHOOT_FREQ(Shoot_Freq)) == 0  && VisionData.RawData.mode!= false /*&& DevicesGet_State(VISION_MONITOR) != Off_line*/)
        {
			...
			break;
case Attack_Unload:
        if((Mshoot_Cnt%=SHOOT_FREQ(10)) == 0 && ContLaunch == true)
        {
            Set_ReloadNum(1);
        }
        break;		
...
//		if(cooling_limit - cooling_heat < 11)
//		{
//			Set_ReloadNum(0);
//			get_time = 0;
//		}

//		if(cooling_limit - cooling_heat < 35 && cooling_limit - cooling_heat > 11 )  // 射频为2以下 和 非狂暴模式下限制热量
//		{
//			waiting_time = 140;
//		}
//		else
...
//    //--- 先执行上次被激活的卡弹任务
    ...
    if(Reload.Num > 0)
    {		
//				if(cooling_limit - cooling_heat < 11)// --- 不满足连发状态
//					{
//						Reload.Num = 0;
//						Reload_Reset();
//							
//						return;
//					}
//         else if(cooling_limit - cooling_heat < 21 && ContLaunch == false)
//         {
//            Reload.Num = 0;
//            return;
//         }
					
//        if(waiting_time != 0)  //新加拨弹延时
//				{
//					get_time++;
//					if(get_time > waiting_time)
//					{
//						get_time = waiting_time;
//					}
//					if(get_time == waiting_time)
//					{
//						Set_ReloadNum(1);
...
uint16_t mag_on = 4700;

uint16_t mag_on_L = 3950;
uint16_t mag_off_L = 1300;
void Mag_Ctrl(uint8_t state)
{
    if (state == 1)
	{
		__HAL_TIM_SetCompare(&htim8, TIM_CHANNEL_2, mag_on);//左		
//		__HAL_TIM_SetCompare(&htim8, TIM_CHANNEL_3, mag_on_L);//右

		Mag_Switch = 1;
	}
	else
	{
		__HAL_TIM_SetCompare(&htim8, TIM_CHANNEL_2, mag_off);//左		
//		__HAL_TIM_SetCompare(&htim8, TIM_CHANNEL_3, mag_off_L);//右
